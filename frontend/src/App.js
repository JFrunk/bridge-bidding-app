...
function App() {
  ...
  const resetAuction = (dealData, skipInitialAiBidding = false) => {
    ...
    // FIX: Store all hands from deal response to prevent state sync issues
    // This ensures "Show All Hands" uses locally stored data instead of server state
    ...
    } else if (alwaysShowHands) {
      // Fallback to fetching if all_hands not in response (shouldn't happen with new backend)
      fetchAllHands();
    } else {
      setAllHands(null);
    }
  ...
  const handleShowHandsThisDeal = async () => {
    ...
    if (!showHandsThisDeal) {
      // FIX: Only fetch if we don't already have all hands stored
      // This prevents state sync issues when server state differs from frontend
      if (!allHands) {
...
${otherCommands}`;
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/api/play-card`, {
        ...
        body: JSON.stringify({
          ...
          user_id: userId || 1  // Include user_id for play evaluation tracking
        })
      });
  ...
  const playRandomHand = async () => {
    try {
      ...
      // FIX: Store all hands from response to prevent state sync issues
      // This ensures "Show All Hands" after play uses locally stored data
      if (data.all_hands) {
        console.log('✅ Storing all_hands from playRandomHand response');
        setAllHands(data.all_hands);
        // Also store in initialDeal for replay functionality
        setInitialDeal({ ...data, all_hands: data.all_hands });
      }
  ...
  const replayCurrentHand = async () => {
    ...
    try {
      ...
      // FIX: Use stored all_hands from initialDeal instead of fetching from server
      // This prevents state sync issues where server state may differ from frontend
      if (initialDeal?.all_hands) {
        console.log('✅ Using stored all_hands from initialDeal for replay');
        setAllHands(initialDeal.all_hands);
        // Hand already set from visible_hands above, no need to fetch
      } else {
        // Fallback: Clear all hands if not available (shouldn't happen with new backend)
        console.log('⚠️ No stored all_hands available, clearing');
        setAllHands(null);
      }
