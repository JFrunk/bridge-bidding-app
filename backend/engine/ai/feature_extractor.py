...
def extract_features(hand: Hand, auction_history: list, my_position: str):
    ...
    # Initialize all auction variables to a default state
    opening_bid = None
    opener = None
    opener_relationship = None
    opener_index = -1
    partner_bids = []
    opener_bids = []
    is_contested = False
    ...
    north_south_bids = False
    east_west_bids = False
    ...
    for i, bid in enumerate(auction_history):
        ...
        if bidder == partner_position:
            partner_bids.append(bid)
        if bidder == opener:
            opener_bids.append(bid)
    
    if north_south_bids and east_west_bids:
        is_contested = True
    ...
    return {
        ...
        'auction_features': { 'num_bids': len(auction_history), 'opening_bid': opening_bid, 'opener': opener, 'opener_relationship': opener_relationship, 'partner_bids': partner_bids, 'partner_last_bid': partner_last_bid, 'opener_last_bid': opener_last_bid, 'opener_index': opener_index, 'is_contested': is_contested },
